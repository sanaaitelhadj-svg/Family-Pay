...
const NAV = [
  ...
  { to: '/qr', label: 'Mon QR', icon: '📱' },
  { to: '/transactions', label: 'Historique', icon: '📋' },
  { to: '/fund-request', label: 'Demande', icon: '💰' },
];
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <header className="bg-emerald-700 text-white px-4 py-3 flex items-center justify-between shadow">
        <div>
          ...
          <span className="ml-2 text-emerald-300 text-sm">Bénéficiaire</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-sm text-emerald-200">{user?.firstName}</span>
          <button
            onClick={logout}
            className="text-xs bg-emerald-600 hover:bg-emerald-500 px-2 py-1 rounded"
          >
            Déconnexion
          </button>
        </div>
        ...
        {NAV.map(({ to, label, icon }) => {
          ...
          return (
            <Link
              key={to}
              to={to}
              className={`flex flex-col items-center text-xs gap-0.5 px-3 py-1 rounded-lg transition ${
                active ? 'text-emerald-700 font-semibold' : 'text-gray-500'
              }`}
            >
